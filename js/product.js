...
$.ajax({
 ...
 // Fetch NestAway Store Products
$.ajax({
    url : "products_estore.php",
    type : "get",
    async: false,
    success : function(data) {
        JSONEstore = JSON.parse(data);
 $.each(JSON.parse(data), function(i, obj){
		$('.home_product_page').append(productcardhtml(obj));
	});
 },
 ...
 allproducts = allproducts.concat(JSONHomes);
  if(JSONEstore!==undefined)
 allproducts = allproducts.concat(JSONEstore);
...
if(filename=='product-detail.php')
{
    ...
    else if(urlparameters.prodCatName == "Estore")
    {
        relatedProducts = JSONEstore;
    }
